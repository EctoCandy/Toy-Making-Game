...
func _build_ui() -> void:
	...
	button_sprite.texture = load("res://Assets/Art/BigScrew.png")
	button_sprite.position = Vector2(215,215)
	panel.add_child(button_sprite)

	click_button = Button.new()
	click_button.name = "ClickButton"
	click_button.position = Vector2(-64, -64)
	click_button.size = Vector2(128, 128)
	click_button.modulate = Color(1.0, 1.0, 1.0, 0.0)
	#click_button.text = "CLICK!"
	click_button.pressed.connect(_on_click_pressed)
	button_sprite.add_child(click_button)
...
func _on_click_pressed() -> void:
	...
	clicks += 1
	button_sprite.rotation += deg_to_rad(12.5)
