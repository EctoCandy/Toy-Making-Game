...
func _build_ui() -> void:
	...
	point_button = TextureButton.new()
	point_button.name = "PointButton"
	point_button.size = point_size
	point_button.texture_normal = load("res://Assets/Art/Screw.png")
	# point_button.text = "●"
...
func _spawn_point() -> void:
	var min_x := 19.0 + point_size.x
	var max_x := panel.size.x - point_size.x - 40.0

	var min_y := 106.0 - point_size.y
	var max_y := panel.size.y - point_size.y - 40.0
